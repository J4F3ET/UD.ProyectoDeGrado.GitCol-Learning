export class SocketHandler {
	...
	async updateCommitsToRepository(data) {
		...
		const sessionStorageCurrent = JSON.parse(
			sessionStorage.getItem(this._remoteRepository)
		);
		sessionStorageCurrent.commits = data;
		sessionStorage.setItem(
			this._remoteRepository,
			JSON.stringify(sessionStorageCurrent)
		);
	...
	async sendUpdateRepository(data) {
		if ("commits" in data) this.client.emit("updateRepository", data.commits);
	}
	async disconnect() {
