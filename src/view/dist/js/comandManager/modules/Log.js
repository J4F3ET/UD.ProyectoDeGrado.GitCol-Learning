...
export class Log {
	...
	async execute(dataComand) {
		...
		const listCommits = await this.resolveFiltersCommits(
			storage.commits,
			startPoint
		);
		const setListCommits = new Set(
			await Promise.all(listCommits.map(async (commit) => commit.id))
		);
		storage.commits = await Promise.all(
			storage.commits.map(async (commit) => {
				if (setListCommits.has(commit.id)) commit.class.push("logging");
				return commit;
			})
		);
		...
		if (this._configurations.n.use) {
			if (this._configurations.n.value > childs.length)
				createMessage(
					this._logRepository,
					"error",
					"The number of commits is greater than the number of commits the log has"
				);
			else childs = childs.slice(0, this._configurations.n.value);
		}
		return childs;
	}
	/**
	...
	async generatorMessage(infoHead, commits) {
		...
		new Set(commits).forEach(async (commit) => {
			...
			commit.tags.forEach(async (tag) => {
				if (tag === "HEAD") {
					const head = infoHead.includes("detached")
						? infoHead.split(" ").pop()
						: infoHead;
					tags += `<b>HEAD</b>&nbsp;->&nbsp;${head},&nbsp`;
				} else if (tag !== infoHead) tags += `${tag},&nbsp`;
			});
			message += `<p class="help underline">Commit:&nbsp;${commit.id}&nbsp;(${tags})</p>`;
			message += `<p class="help">Author:&nbsp;${
				commit.author ? commit.author.name : undefined
			}&nbsp;&lt;${commit.author ? commit.author.email : undefined}&gt;</p>`;
