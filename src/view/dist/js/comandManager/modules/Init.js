...
export class Init {
	...
	async resolveConfiguration(config) {
		...
		const clearConfig = config.map((conf) =>
			conf.replace(/^--?/, "").charAt(0)
		);
		clearConfig.forEach((conf) => {
			if (conf in this._configurations) this._configurations[conf].callback();
			else throw errorNotConfiguration(this._comand, conf);
		});
	}
	/**
	 * @name callbackHelp
