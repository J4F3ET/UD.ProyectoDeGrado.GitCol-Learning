...
export class Commit {
	...
	async execute(dataComand) {
		...
		else head = await removeTags(["HEAD"], head);
	...
	async validateConfig(configs) {
		if (!configs.size)
			throw new ErrorModule(
				...
				`Please, try again using the command 'git ${this._comand} -m "message"'`
			);
	...
	callBackConfigFiles = async (dataComand) => {
		...
		const files = this._configurations.a.files
			.map((file) => `<li>>${file}</li>`)
			.join("");
		createMessage(
			this._logRepository,
			"info",
			`<div class="files"><h5>Add files to the commit</h5><ul>${files}</ul></div>`
		);
