...
async function implementTagsRemotesInRepository(
	...
	return await Promise.all(
		commits.map(async (commit, _, commits) => {
 ...
 */
async function resolveTagsInChange(
	commit,
	commits,
	commitsHeadRemote,
	changeMap,
	...
	if (commitsHeadRemote.has(commit.id)) {
		...
		commits.forEach((commitForEach) => {
			...
			changeMap.get(commit.id).forEach((tagMap) => {
				commitForEach.tags = commitForEach.tags.filter(
					(t) => t != refRemote + "/" + tagMap
				);
			});
		});
	}
...
async function findAllTags(commits) {
	const allTags = (
		await Promise.all(commits.map(async (commit) => commit.tags || []))
	).flat();
...
async function findAllChildrens(commits, id, childrens = []) {
	const childrensFisrtGen = commits.filter(
		(commitStorage) => commitStorage.parent == id
	);
	if (childrensFisrtGen.length == 0) return childrens;
...
async function findAllParents(commits, commit, parents = []) {
	if (!commit) return [];
	const parentsCommits = commits.filter(
		(comt) => comt.id == commit.parent || commit.unions?.includes(comt.id)
	);
	if (parentsCommits.length === 0) return parents;
 ...
 */
async function findCommitsDiffBetweenRepositories(
	commitsDestination,
	commitsOrigin
) {
...
async function resolveCreateMergeRegister(storage, commitFetch, commitHead) {
	...
	} else storage.information.head = "detached to " + commit.id;
...
async function resolveMovilityTagInMerge(storage, commitFetch, commitHead) {
	...
	} else storage.information.head = "detached to " + commitFetch.id;
...
async function resolveIsHeadNull(repository, tagDefault = "master") {
	...
	if (!master) return resolveIsHeadNull(repository, tags.shift() || "master");

	//Si existe HEAD buscara el commit head si no buscara master
	const findConditionalParameter = repository.commits.some((c) =>
		c.tags.includes("HEAD")
	)
		? "HEAD"
		: master;
	const commit = repository.commits.find((c) =>
		c.tags.includes(findConditionalParameter)
	);
...
function removeManyClassFromCommit(commit, classToRemove) {
	return (commit.class = commit.class.filter(
		(classC) => !classToRemove.includes(classC)
	));
}
/**
...
async function existBranchOrEndPoint(commits, branch) {
	...
	const idCommits = new Set(
		await Promise.all(commits.map(async (commit) => commit.id))
	);
...
async function resolveLocationCommit(commits, parentCx, parentCy) {
	...
	const commitsInNegativeY = commits.filter(
		(commit) => commit.cx == possibleX && commit.cy > parentCy
	);

	const commitThisUbicationOnParentY = commits.filter(
		(commit) => commit.cx == parentCx && commit.cy != parentCy
	);
 ...
 */
async function generateLocationCommitCase2(
	parentCy,
	commitsInPossiteY,
	commitsInNegativeY
) {
	if (commitsInPossiteY.length == 0) return parentCy - SPACE_BETWEEN_COMMITS_Y;

	if (commitsInNegativeY.length == 0) return parentCy + SPACE_BETWEEN_COMMITS_Y;

	if (commitsInPossiteY.length <= commitsInNegativeY.length)
		return (
			commitsInPossiteY[commitsInPossiteY.length - 1].cy -
			SPACE_BETWEEN_COMMITS_Y
		);
	else
		return (
			commitsInNegativeY[commitsInNegativeY.length - 1].cy +
			SPACE_BETWEEN_COMMITS_Y
		);
}
/**
 ...
 */
async function generateLocationCommitCase3(
	commits,
	parentCy,
	commitsInPossiteY,
	commitsInNegativeY
 ...
 */
async function updateLocationChildsOfCommit(
	commits,
	SPACE_BETWEEN_COMMITS_Y,
	idCommitParent
) {
...
async function createRegister(commits, parent, information, message) {
	...
	if (
		information.head.includes("detached") ||
		parent.class.includes("detached-head")
	) {
...
async function newRegister(
	...
	return {
		...
		class: classList,
		autor:
			autor ?? JSON.parse(sessionStorage.getItem("config")).user.autor ?? null,
...
function findAllExceptionCommitsToDelete(commits) {
	const parents = commits
		.map((commit) =>
			commit.parent && !commit.class.includes("detached-head")
				? commit.parent
				: []
		)
		.flat();
	if (parents.length === 0)
		return commits.filter(
			(commit) => commit.id == "parent" || commit.id == "init"
		);
	const repeatedParents = parents.filter(
		(parent, index) => parents.indexOf(parent) != index
	);
...
function deleteCommitsRecursivelyUntil(commits, commitObj, pointsObjetive) {
	if (pointsObjetive.includes(commitObj.id)) return commits;
	const parent = commits.find((commit) => commit.id === commitObj.parent);
	commits = commits.map((commit) => {
		if (commit.id === commitObj.id) commitObj.class.push("detached-head");
		return commit;
	});
...
async function changeDetachedCommitToCommit(commit, commits) {
	if (!commit || !commit.class.includes("detached-head")) return commits;
	let parent;
	const newListCommits = await Promise.all(
		commits.map(async (c) => {
			if (c.id == commit.id)
				c = await removeClassFromCommit(commit, "detached-head");
			if (c.id == commit.parent) parent = c;
			return c;
		})
	);
	await commit.unions.forEach(async (union) => {
		const commitUnion = newListCommits.find((c) => c.id == union);
		newListCommits = await changeDetachedCommitToCommit(
			commitUnion,
			newListCommits
		);
	});
 ...
 */
async function findChangesBetweenBranchs(
	commitsDestination,
	commitsOrigin,
	nameBranch
) {
	const commitHeadOrigin = commitsOrigin.find((commit) =>
		commit.tags.includes(nameBranch)
	);
	const historyBranchOrigin = [
		...(await findAllParents(commitsOrigin, commitHeadOrigin)),
		commitHeadOrigin,
	];
 ...
 */
async function mergeChangesInBranchs(
	commitsDestination,
	commitsOrigin,
	nameBranch
) {
	const commitsChanges = await Promise.all(
		(
			await findChangesBetweenBranchs(
				commitsDestination,
				commitsOrigin,
				nameBranch
			)
	...
	return {
		changesId: new Set(
			await Promise.all(commitsChanges.map(async (c) => c.id))
		),
...
async function addChangesRecursivelyToRepository(commits, changes) {
	if (changes.length == 0) return commits;
	const commitIdSet = new Set(
		commits.length != 0 ? commits.map((commit) => commit.id) : []
	);
	...
	return await tags.reduce(async (accPromise, t) => {
		...
		if (acc.changeMap.has(commitHeadOrigin.id)) {
			for (const commit of acc.repository) {
				if (commitHeadOrigin.id != commit.id) continue;
				commit.tags.push(t);
	...
	for (const commit of commits) {
		if (refRemote && commit.tags.length) {
			commit.tags.forEach((t) => {
				if (t.includes(refRemote)) heads.add(commit.id);
			});
 ...
 */
async function findCommitsEqualBetweenRepositories(
	commitsDestination,
	commitsOrigin
) {
	const destinationCommitId = new Set(
...
async function findCommitLink(idPotentialParents, idParentsOfChildrens) {
	return (
		idParentsOfChildrens.find((idParentOfChild) =>
			idPotentialParents.includes(idParentOfChild)
		) || null
	);
}
export {
