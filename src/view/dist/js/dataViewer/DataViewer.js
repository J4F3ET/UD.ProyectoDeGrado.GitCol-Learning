...
export class DataViewer {
	...
	notify(data) {
		if (data == null) return;
		if (Object.hasOwn(JSON.parse(data), "commits")) this.updateSVG(data);
	...
	initRepository(svgDocumentElement) {
		if (!svgDocumentElement) return;
		svgDocumentElement.innerHTML = "";
		const gContainerData = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"g"
		);
		gContainerData.id = "gContainerData";
		const gContainerPointer = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"g"
		);
		gContainerPointer.id = "gContainerPointer";
		const gContainerTag = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"g"
		);
		gContainerTag.id = "gContainerTag";
		const gContainerCommit = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"g"
		);
	...
	createTag(x, y, tagName) {
		...
		if (tagName == "HEAD") newTag.classList.add("head-tag");
	 ...
	 */
	resizeSVG() {
		if (this._svg.querySelectorAll(".commit").length == 0) return;
		const maxCX =
			Math.max(
				...Array.from(this._svg.querySelectorAll(".commit")).map((commit) =>
					parseInt(commit.getAttribute("cx"))
				)
			) + 150;
		if (maxCX < parseInt(this._svg.getAttribute("width"))) return;
	...
	createMessage(log) {
		...
		if (log.tag == "comand") p.innerHTML = "$ " + p.innerHTML;
	 ...
	 */
	updateLog(data) {
		if (data == this._logComands) return;
	...
	updateSVG(data) {
		...
		const currentCommitsData =
			currentData == null ? [] : currentData.commits ?? [];
		const currentInfoData =
			currentData == null ? {} : currentData.information ?? {};
	...
	renderCommitsToSVG(commitsData, currentCommits = null) {
		if (!currentCommits) {
			commitsData.forEach((commit) => {
				const parent =
					commitsData.find((c) => c.id === commit.parent) ?? this._commitParent;
				const unions =
					commit.unions?.length > 0
						? this.getAllUnionsToCommit(commitsData, commit)
						: [];
	...
	removeLineFromSVG(idsCommits) {
		const linesInSVG = Array.from(
			this._svg.querySelectorAll(".line,.union")
		).map((line) => line.id);
		linesInSVG.forEach((line) => {
			const ids = line.split("-");
			if (
				!idsCommits.includes(ids[1]) ||
				(!idsCommits.includes(ids[0]) && ids[0] != "init")
			)
	...
	removeTagsFromSVG(tagsInData) {
		const tagsInSVG = Array.from(this._svg.querySelectorAll(".branch-tag")).map(
			(tag) => tag.id
		);
	...
	removeCommitsFromSVG(idsCommits) {
		const commitInSVG = Array.from(this._svg.querySelectorAll(".commit")).map(
			(commit) => commit.id
		);
	...
	renderInfoToSVG(data) {
		...
		keys.forEach((key, index) => {
			...
			const gContainerText = document.createElementNS(
				"http://www.w3.org/2000/svg",
				"g"
			);
	...
	resolveLocationOfTextByInfoSVG(xKey, value) {
		...
		return (
			widthText + (widthText > 100 ? xKey * 1.35 : xKey * multiploMinWidthText)
		);
	}
	/**
	...
	resolveTagsInSVG(commit) {
		...
		commit.tags.forEach((tag, index) => {
			...
			else this.addTagToSvg(this.createTag(commit.cx, y + index * 25, tag));
		});
	}
	/**
	 * @name addCommitToSvg
	...
	addCommitToSvg(commit, parent, unions = []) {
		unions.forEach((union) =>
			this.addLineToSvg(this.createLine(commit, union, ["union"]))
		);
	...
	updateTagToSvg(tag, x, y) {
		...
		this.animateElement(
			tagSVG.getElementsByTagName("rect")[0],
			"x",
			"y",
			xRect,
			yRect
