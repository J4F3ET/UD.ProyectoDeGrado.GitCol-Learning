...
const defaultUsageModelSteps = [
	...
	{
		element:
			window.innerWidth > 1106 ? "#logContainer" : "#controlBoxContainer",
		popover: {
			title: "Command log",
...
export const driveUsageMode = (...steps) => {
	if (!steps.length) return getDriver(defaultUsageModelSteps);
...
const helpRooms = [
	...
	},
	{
		...
		popover: {
			...
			description:
				"The code must meet the following requirements. Example 'ASDF'",
			side: "left",
			align: "start",
		},
	},
	{
		...
		popover: {
			title: "Button login",
			description:
				"If the code is valid, the button to click on it will be enabled.Example 'ASDF'",
			side: "left",
			align: "start",
		},
	},
	{
		...
		popover: {
			title: "Search room",
			description:
				"If for some reason you cannot find the room, try searching in the public rooms.",
			side: "left",
			align: "start",
		},
	},
